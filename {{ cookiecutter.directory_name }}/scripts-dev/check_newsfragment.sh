...
python -m towncrier.check --compare-with="origin/{{ cookiecutter.main_git_branch }}" || exit 1
...
matched=0
for f in `git diff --name-only origin/{{ cookiecutter.main_git_branch }}... -- changelog.d`; do
